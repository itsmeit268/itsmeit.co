...
function wp_keyword_tool_timedCount() {
	...
	if( typeof(wp) != "undefined" && wp.hasOwnProperty('data') && wp.data.hasOwnProperty('select') && wp.data.select( "core/editor" ) != undefined){
		...
		//gutenberg
		var content =  jQuery('<span>').html(wp.data.select( "core/editor" ).getEditedPostAttribute( 'content' )).text().toLowerCase();
		
	}else{
		
		//oldMCE
		var content = jQuery('#content_ifr').contents().find('.mceContentBody').text().toLowerCase();
    ...
    jQuery(
        ...
        .each(
            function () {
                ...
                count = content.match(RegExp( '\\b' +  word + '\\b' ,  'g'));
...
jQuery(document)
    .ready(
        function () {
                            ...
                            .each(
                                function () {
                                    ...
                                    jQuery(
                                        ...
                                        .append(
                                            '<div class="wp_keyword_tool_itm"><span><a  class="dashicons-before dashicons-dismiss"></a></span><div class="wp_keyword_tool_keyword">' + jQuery(
                                                this)
            ...
            jQuery('.wp_keyword_tool_itm a').click(function(){
