<?php
	...
	/*  Copyright 2012-2023  Wordpress Pinterest Automatic  (email : sweetheatmn@gmail.com) */

	update_option('wp_pinterest_automatic_license', '************');
							...
							if(trim($tocken) != ''){
								...
								foreach($instantpinimages as $img){
									...
									if( stristr($pintext, 'post_excerpt') ) {
										...
										//rank math support field rank_math_description
										if( isset($_POST) && array_key_exists('rank_math_description',$_POST) &&  trim($_POST['rank_math_description']) != ''){
											$thepost->post_excerpt = trim($_POST['rank_math_description']);
										}

										//if excerpt is empty and function rank_math is defined, get the excerpt from rank math field rank_math_description
										if(trim($thepost->post_excerpt) == '' && function_exists('rank_math')){
											$possible_excerpt = get_post_meta($thepost->ID,'rank_math_description',1);
			...
			*/
			...
			if( (trim($publish) == 'publish' &&  isset($_POST['post_date']) ) || $publish == 'draft'  || $publish == 'future'  ){
				...
				if(is_array($pin_options)){
					...
					if( in_array('OPT_PIN',$pin_options) && isset($_POST['pin_images']) && is_array($_POST['pin_images']) ){
	...
	function wp_pinterest_automatic_parse_request($wp) {
		...
		if (array_key_exists('wp_pinterest_automatic', $wp->query_vars)) {
			...
			if($wp->query_vars['wp_pinterest_automatic'] == 'boards'){
				...
				// if contains { , json decode
				if(strpos($exec, '{') !== false){
